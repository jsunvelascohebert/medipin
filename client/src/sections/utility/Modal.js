  ...
  return (
    <>{isModalOpen ? (
      ...
      <div tabIndex={-1} aria-hidden="true" className={`fixed top-0 left-0 right-0 z-50 flex justify-center items-center w-full h-full overflow-x-hidden overflow-y-hidden bg-${color} bg-opacity-75 hover:cursor-pointer`} onClick={toggleModalState}>
        ...
        {/* inner container */}
        <div className={`flex flex-col max-h-3/4 w-4/5 justify-start items-center bg-${color} border-2 border-${darkColor} shadow-md shadow-${darkColor} rounded-xl cursor-auto`} onClick={(event) => event.stopPropagation()}>
          ...
          <div className={`w-full flex flex-col sm:flex-row justify-between items-center border-t-2 p-4 border-${darkColor} gap-2 sm:gap-0`}>
            {/* go back */}
            <a className={`group flex flex-row justify-start items-center gap-2 hover:cursor-pointer text-${darkColor}`} onClick={toggleModalState}>
